...
export class FrontendHostingConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    // 1) Client-rendered dynamic route: every `/analyze/analysis-results/<id>` deep
    //    link is served the single prerendered shell (generated by
    //    generateStaticParams with sessionId 'placeholder'). The app reads the real
    //    id from the URL at runtime. NOTE: the 'placeholder' value here must match
    //    generateStaticParams in app/analyze/analysis-results/[sessionId]/page.tsx.
    // 2) Directory requests: the static export uses `trailingSlash: true`, so routes
    //    are stored as `route/index.html`. Rewrite "directory" requests (ending in
    //    `/`, or extension-less paths like `/config`) to the matching index.html.
    const directoryIndexFunction = new cloudfront.Function(this, 'DirectoryIndexRewrite', {
      ...
      code: cloudfront.FunctionCode.fromInline(
        [
          ...
          '  var uri = request.uri;',
          "  if (uri.indexOf('/analyze/analysis-results/') === 0) {",
          "    request.uri = '/analyze/analysis-results/placeholder/index.html';",
          '    return request;',
          '  }',
    ...
    this.distribution = new cloudfront.Distribution(this, 'FrontendDistribution', {
      ...
      // Graceful fallback for genuinely missing paths (S3 returns 403 under OAC).
      // The dynamic results route is handled by the function rewrite above, so this
      // mainly covers mistyped URLs, which land on the app's home page.
      errorResponses: [
        {
          httpStatus: 403,
